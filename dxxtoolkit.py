...
def dxx_process_game_info_response(data, version, proto):
    ...
    if len(data) == 73:
        ...
        game_data['game_id'] = unpacked_data[4]
        game_data['netgame_name'] = re.sub(r'[^a-zA-Z0-9 -_]','',unpacked_data[5].decode(errors='ignore').replace('\x00', ''))
        game_data['mission_title'] = unpacked_data[6].decode(errors='ignore').replace('\x00', '')
        game_data['mission_name'] = unpacked_data[7].decode(errors='ignore').replace('\x00', '')
        ...
        game_data['netgame_name'] = re.sub(r'[^a-zA-Z0-9 -_]','',unpacked_data[settings_offset].decode(errors='ignore').replace('\x00', ''))
        game_data['mission_title'] = unpacked_data[settings_offset+1].decode(errors='ignore').replace('\x00', '')
        game_data['mission_name'] = unpacked_data[settings_offset+2].decode(errors='ignore').replace('\x00', '')
